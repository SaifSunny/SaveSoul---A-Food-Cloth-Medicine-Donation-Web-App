...
if (isset($_POST["signin"])) {
    ...
    if ($result->num_rows > 0) {
        $sql = "SELECT * FROM `ngo` WHERE `password`='$password'";
        $result = mysqli_query($conn, $sql);

        if ($result->num_rows > 0) {
            $sql = "SELECT * FROM `ngo` WHERE username='$username' AND password='$password'";
            $result = mysqli_query($conn, $sql);

            $sql1 = "UPDATE ngo SET `status`=1 WHERE username='$username'";
            $result1 = mysqli_query($conn, $sql1);

            if ($result->num_rows > 0) {
                $_SESSION["ngoname"] = $_POST["username"];
                $sql = "UPDATE ngo SET `status`=1 WHERE username='$username'";
                $result = mysqli_query($conn, $sql);
                if($result){
                  header("Location: ngo_home.php");
                }
