...
if (isset($_POST['signin'])) {
	...
	if ($result->num_rows > 0) {
        ...
        $sql = "SELECT * FROM donners WHERE `password`='$password'";
        $result = mysqli_query($conn, $sql);
    
        if ($result->num_rows > 0) {
            $sql = "SELECT * FROM donners WHERE username='$username' AND `password`='$password'";
            $result = mysqli_query($conn, $sql);
        
            if ($result->num_rows > 0) {
                $_SESSION['donnername'] = $_POST['username'];
                #setting active status to 1
                $sql = "UPDATE donners SET `status`=1 WHERE username='$username'";
                $result = mysqli_query($conn, $sql);
                if($result){
                  header("Location: donner_home.php");
                }
...
<body class="text-center">
  ...
  <main class="form-signin">
    <form action="" method="post">
      ...
      <p class="mt-5 mb-3 text-muted">New User? Create an Account <a href="donner_signup.php">Now</a></p>
    </form>
  </main>
